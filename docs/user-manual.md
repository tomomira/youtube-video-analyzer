...
- **最小再生回数**: この回数**以上**の動画を検索（下限）
- **最大再生回数**: この回数**以下**の動画を検索（上限）

**例**:
- 最小200万 → 200万回以上の動画（人気動画に絞る）
- 最大200万 → 200万回以下の動画（あまり人気でない動画も含む）
- 両方設定 → 指定範囲内の動画のみ

```
例1: バズった動画を探す（人気動画のみ）
  最小再生回数: 1000000（100万回以上）
  最大再生回数: （空欄 = 上限なし）
  → 100万回以上の動画だけが表示される

例2: 隠れた良動画を探す（再生回数が少ない動画）
  最小再生回数: 1000（1000回以上）
  最大再生回数: 10000（1万回以下）
  → 1000回〜1万回の範囲の動画だけが表示される

例3: 特定の範囲で絞り込む
  最小再生回数: 100000（10万回以上）
  最大再生回数: 500000（50万回以下）
  → 10万回〜50万回の範囲の動画だけが表示される
```
